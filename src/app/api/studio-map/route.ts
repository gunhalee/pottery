...
const staticMapEndpoint = "https://maps.apigw.ntruss.com/map-static/v2/raster";
  ...
  try {
    ...
    if (!mapResponse.ok) {
      return createMapPlaceholder(
        `네이버 Static Map API 응답 오류: ${mapResponse.status}`,
      );
    }
    ...
    return createMapPlaceholder("네이버 Static Map API 요청에 실패했습니다.");
  }
}
